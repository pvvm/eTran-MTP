...
#define QUEUE_ID 0
#define FILL_RING_SIZE (XSK_RING_PROD__DEFAULT_NUM_DESCS * 2) // recommened to be RX_RING_SIZE + NIC RING SIZE
 ...
 * @note: (RX/TX/FILL/COMP_RING_SIZE, NUM_FRAMES, FRAME_SIZE) need negotiating with privileged processes
 */
int create_afxdp_socket(struct xsk_socket_info *xsk_info, int xsk_fd)
{
    ...
    rx->producer = xsk_info->rx_map + off.rx.producer;
    rx->consumer = xsk_info->rx_map + off.rx.consumer;
    rx->flags = xsk_info->rx_map + off.rx.flags;
    ...
    tx->producer = xsk_info->tx_map + off.tx.producer;
    tx->consumer = xsk_info->tx_map + off.tx.consumer;
    tx->flags = xsk_info->tx_map + off.tx.flags;
    ...
    xsk_info->fill_map = mmap(NULL, xsk_info->fill_map_size, PROT_READ | PROT_WRITE,
    ...
    fill->producer = xsk_info->fill_map + off.fr.producer;
    fill->consumer = xsk_info->fill_map + off.fr.consumer;
    fill->flags = xsk_info->fill_map + off.fr.flags;
    ...
    xsk_info->comp_map = mmap(NULL, xsk_info->comp_map_size, PROT_READ | PROT_WRITE,
    ...
    comp->producer = xsk_info->comp_map + off.cr.producer;
    comp->consumer = xsk_info->comp_map + off.cr.consumer;
    comp->flags = xsk_info->comp_map + off.cr.flags;
...
static inline void complete_tx_l2fwd(struct xsk_socket_info *xsk)
{
	...
	ndescs = (xsk->outstanding_tx > 32) ? 32 :
		xsk->outstanding_tx;
	...
	if (rcvd > 0) {
		...
		while (ret != rcvd) {
			if (ret < 0)
				exit(EXIT_FAILURE);
			if (xsk_ring_prod__needs_wakeup(&xsk->fq)) {
				recvfrom(xsk->xsk_fd, NULL, 0, MSG_DONTWAIT, NULL,
					 NULL);
			}
			ret = xsk_ring_prod__reserve(&xsk->fq, rcvd, &idx_fq);
		}
...
void *dump_stats(void *arg)
{
    ...
    while (1)
    {
        printf("Rx pps: %.2f Mpps, Tx pps: %.2f Mpps\n", 
            (nr_rx_packets - prev_nr_rx_packets) / 1e6, (nr_tx_packets - prev_nr_tx_packets) / 1e6);
...
int main() {
    ...
    ret = create_afxdp_socket(&xsk_info, xsk_fd);
    if (ret) {
    ...
    pthread_create(&dump_thread, NULL, dump_stats, NULL);
