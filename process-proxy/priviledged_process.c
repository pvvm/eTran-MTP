...
int main() {
    ...
    /* Note: Actually, xsk_socket__fd(xsk) == xsk_umem__fd(umem). This is because libxdp would reuse the UMEM's fd 
     * for the ***first socket*** binding to the UMEM, the UMEM fd is also created by socket(AF_XDP, SOCK_RAW, 0).
     * See xsk_socket__create_shared()
    */
    assert(xsk_socket__fd(xsk) == xsk_umem__fd(umem));

    // Step4: send the file descriptors for the AF_XDP socket
    if (send_fd(client_sock, xsk_socket__fd(xsk)))
        goto out;  
