...
{
	struct bpf_map *rx_cnt = (struct bpf_map *)arg;
	__u32 key = 0;
	__u64 cnt;
	__u64 old_cnt = 0;
	while (1) {
		sleep(1);
		int err = bpf_map_lookup_elem(bpf_map__fd(rx_cnt), &key, &cnt);
		if (err) {
			pr_warn("Couldn't get RX count: %s\n", strerror(-err));
			return NULL;
		}
		pr_info("Packet rate: %.2f mpps\n", (cnt - old_cnt) * 1.0 / 1000000);
		old_cnt = cnt;
...
int do_udp(const void *opt, __unused const char *pin_root_path)
{
	...
	pthread_create(&stats_thread, NULL, sample_func, skel->maps.rx_cnt);
