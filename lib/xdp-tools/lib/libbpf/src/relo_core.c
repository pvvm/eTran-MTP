...
recur:
	...
	if (!btf_kind_core_compat(local_type, targ_type))
	{
		printf("local_type->info = %u, targ_type->info = %u\n", 
		local_type->info, targ_type->info);
		printf("btf_kind(local_type) = %d, btf_kind(targ_type) = %d\n",
		btf_kind(local_type), btf_kind(targ_type));
		return 0;
	}
