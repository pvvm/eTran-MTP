			      ...
			      const struct xsk_socket_config *usr_config)
{
	...
	// XDP_EGRESS
	if (usr_config->xdp_egress_fd > 0)
	{
		sxdp.sxdp_xdp_egress_prog_fd = usr_config->xdp_egress_fd;
		sxdp.sxdp_flags |= XDP_EGRESS;
		printf("libxdp: XDP_EGRESS(fd:%d) enabled at %s:%d\n", usr_config->xdp_egress_fd, ifname, queue_id);
	}
	// XDP_EGRESS end

	// XDP_GEN
	if (usr_config->xdp_gen_fd > 0)
	{
		sxdp.sxdp_xdp_gen_prog_fd = usr_config->xdp_gen_fd;
		sxdp.sxdp_flags |= XDP_GEN;
		printf("libxdp: XDP_GEN (fd:%d) enabled at %s:%d\n", usr_config->xdp_gen_fd, ifname, queue_id);
	}
	// XDP_GEN end
