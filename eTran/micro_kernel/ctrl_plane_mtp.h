...
void mtp_set_rate(uint32_t cc_idx, uint32_t new_rate) {
    etran_tcp->_tcp_cc_map_mmap->entry[cc_idx].rate = new_rate;
}
...
static inline void ack_timeout_ep(struct timer_event *ev, struct tcp_connection *c, struct interm_out *int_out) {
    uint32_t cur_ts = cycles_to_us(ev->curr_tsc);
    uint32_t rtt = (c->rtt_est ? c->rtt_est : TCP_RTT_INIT);

    if (c->txp && c->cnt_rx_ack_bytes == 0)
    {
                 ...
                 (cur_ts - c->ts_tx_pending) >= std::max(REXMIT_INTS * rtt, (unsigned int)TCP_RTO_MIN))
        {
            ...
            c->cc_rexmits++;
            issue_retransmit(c);
            // Question: here would be the border between control path and XDP for the compiler.
            // Everything (in this scope) onwards is in XDP. Does this sound reasonable?
            // And issue_retransmit(c) will be generated by default when the compiler detects
            // that border.
        }
    }
    else
