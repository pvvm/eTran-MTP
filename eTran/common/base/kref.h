...
{
	assert(atomic32_read(&ref->cnt) > 0);
	atomic32_inc(&ref->cnt);
}
...
{
	assert(release);
	if (atomic32_dec_and_test(&ref->cnt))
		release(ref);
}
...
{
	return atomic32_read(&ref->cnt) == 0;
}
